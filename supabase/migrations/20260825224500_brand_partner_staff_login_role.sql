...
begin
  ...
  where n.nspname = 'public' and t.relname = 'login_users'
    and c.conname = 'login_users_role_check' and c.contype = 'c';

  if v_existing_check is not null then
    execute 'alter table public.login_users drop constraint login_users_role_check';
    execute format('alter table public.login_users add constraint login_users_role_check check ((%s) or lower(trim(coalesce(role, ''''))) = ''brand partner'')', v_existing_check);
  else
    alter table public.login_users add constraint login_users_role_check
      check (lower(trim(coalesce(role, ''))) in ('admin','administrator','accounts','accountant','sales rep','sales representative','driver','warehouse','brand partner','partner','super admin','customer')) not valid;
  end if;
end;
$$;
...
create or replace function public.fc_save_staff_login_v1(
  ...
  p_target_username text, p_new_password text, p_role text, p_staff_active boolean,
...
begin
  ...
  if v_role not in ('Admin','Accounts','Accountant','Sales Rep','Driver','Warehouse','Brand Partner','Super Admin') then
  ...
  if p_target_login_id is not null then
    select * into v_target from public.login_users where id = p_target_login_id and staff_id = p_target_staff_id and lower(trim(coalesce(role,''))) <> 'customer';
  else
    select * into v_target from public.login_users where staff_id = p_target_staff_id and lower(trim(coalesce(role,''))) <> 'customer' order by created_at desc nulls last limit 1;
  end if;
  ...
  if nullif(lower(trim(coalesce(p_target_username,''))),'') is null then raise exception 'A staff username is required.' using errcode='22023'; end if;
  if v_target.id is null and length(coalesce(p_new_password,'')) < 8 then raise exception 'A password of at least 8 characters is required for a new staff login.' using errcode='22023'; end if;
  if p_new_password is not null and length(p_new_password) < 8 then raise exception 'The new password must be at least 8 characters.' using errcode='22023'; end if;
  ...
  if v_target.id is null then
    ...
    values(p_target_staff_id,lower(trim(p_target_username)),null,extensions.crypt(p_new_password,extensions.gen_salt('bf',12)),v_role,null,coalesce(p_login_enabled,true),now()) returning * into v_target;
  else
    update public.login_users
    set username=lower(trim(p_target_username)), role=v_role, active=coalesce(p_login_enabled,true),
  ...
  if p_login_enabled is false or p_staff_active is false then
    update public.fc_login_sessions set revoked_at=now(), revoked_reason='STAFF_OFFBOARDED' where login_id=v_target.id and revoked_at is null;
  end if;
