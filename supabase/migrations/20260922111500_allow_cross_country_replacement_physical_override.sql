-- Restore warehouse physical-pick override with activity audit.
-- Apply to TEST/V3 first. Confirmed physical stock may be packed even when the
-- tracked country inventory row is missing or insufficient. Inventory is never
-- driven negative and missing location rows are not invented by the override.
...
declare
  ...
  v_missing_country_stock boolean := false;
  v_insufficient_country_stock boolean := false;
...
begin
  ...
  -- A confirmed in_stock action means the picker has the physical product.
  -- Tracked stock mismatches must be audited, not used to block packing.
  v_allow_stock_mismatch := p_action in ('in_stock', 'replace');
  ...
  if p_action <> 'pre_order' then
    if p_action <> 'replace'
       and public.fc_normalize_inventory_country_v1(p_inventory_country)
    ...
    if p_action = 'replace' then
      v_country := public.fc_normalize_inventory_country_v1(p_inventory_country);
      if v_country not in ('England', 'Wales') then
        raise exception 'Replacement inventory country must be England or Wales.'
          using errcode = '22023';
      end if;
    end if;
    ...
    if v_country_location_count = 0 then
      ...
      if v_any_location_count > 0 then
        if p_action in ('in_stock', 'replace') then
          -- Physical stock wins for packing. Do not invent a country stock row;
          -- record the pick with no stock deduction and surface it in activity.
          v_missing_country_stock := true;
        else
          raise exception
      ...
      if not v_missing_country_stock then
        select count(*)
        into v_country_location_count
    ...
    if not found and not v_missing_country_stock then
      raise exception
        'Could not initialise the % inventory row for this product.',
        v_country;
    end if;

    if not v_missing_country_stock and v_stock_qty is null then
      raise exception 'Location stock quantity is null.';
    end if;

    if not v_missing_country_stock and v_stock_qty < 0 then
      raise exception 'Location stock quantity is negative.';
    end if;

    v_insufficient_country_stock :=
      p_action in ('in_stock', 'replace')
      and not v_missing_country_stock
      and coalesce(v_stock_qty, 0) < p_quantity;

    v_is_picking_mismatch :=
      v_is_picking_mismatch
      or v_missing_country_stock
      or v_insufficient_country_stock;
    ...
    v_stock_deduct_qty :=
      case
        when v_missing_country_stock then 0
        when v_allow_stock_mismatch then least(p_quantity, greatest(coalesce(v_stock_qty, 0), 0))
        else p_quantity
      end;

    if v_stock_row_id is not null and v_stock_deduct_qty > 0 then
  ...
  values(
    ...
    case
      when v_missing_country_stock and p_action = 'replace' then 'Picking mismatch: physical replacement found but active country inventory row is missing'
      when v_insufficient_country_stock and p_action = 'replace' then 'Picking mismatch: physical replacement found above tracked country inventory'
      when v_missing_country_stock and p_action = 'in_stock' then 'Picking mismatch: physical stock found but active country inventory row is missing'
      when v_insufficient_country_stock and p_action = 'in_stock' then 'Picking mismatch: physical stock found above tracked country inventory'
