create table if not exists public.brand_promotion_claims (
...
create table if not exists public.brand_promotion_receipts (
  id uuid primary key default gen_random_uuid(),
  claim_id uuid not null references public.brand_promotion_claims(id) on delete restrict,
...
alter table public.brand_promotion_claims enable row level security;
alter table public.brand_promotion_receipts enable row level security;

revoke all on table public.brand_promotion_claims from anon, authenticated;
revoke all on table public.brand_promotion_receipts from anon, authenticated;
grant all on table public.brand_promotion_claims to service_role;
grant all on table public.brand_promotion_receipts to service_role;

create index if not exists brand_promotion_claims_rule_idx
  on public.brand_promotion_claims(promotion_rule_id, claim_date);
create index if not exists brand_promotion_receipts_claim_idx
  on public.brand_promotion_receipts(claim_id, received_date);

create or replace function public.fc_brand_partner_dashboard_v1(
  p_username text,
...
revoke all on function public.fc_brand_partner_dashboard_v1(text,text,date,date,text,text,text,text,integer,integer,integer,integer) from public;
grant execute on function public.fc_brand_partner_dashboard_v1(text,text,date,date,text,text,text,text,integer,integer,integer,integer) to anon, authenticated, service_role;
