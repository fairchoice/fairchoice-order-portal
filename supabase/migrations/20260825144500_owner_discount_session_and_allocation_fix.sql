-- Session-authorised owner discount posting and canonical allocation validation.
-- Additive/forward-only: does not rewrite existing financial rows.
...
begin
  ...
  if nullif(trim(coalesce(p_idempotency_key,'')),'') is not null then
    ...
    where customer_account_id = p_customer_account_id
      ...
      and upper(coalesce(status,'POSTED')) not in ('VOID','VOIDED','REVERSED','CANCELLED','CANCELED')
    ...
    if found then
      ...
      return jsonb_build_object('duplicate',true,'payment',to_jsonb(v_payment),'allocations',v_allocations);
    end if;
  end if;

  -- Validate every allocation against the canonical invoice table used by
  -- the database writer. This keeps the UI preview and database remaining
  -- balance on the same source of truth.
  for v_allocation in
  ...
  loop
    ...
    if v_requested > greatest(0, round(v_invoice.invoice_total - v_allocated,2)) then
  ...
  v_reference := case
    ...
    else 'DISC-' || to_char(coalesce(p_payment_date,now()),'YYYYMMDD') || '-' ||
...
revoke all on function public.post_owner_central_discount_v2(
  text,text,uuid,uuid,timestamptz,numeric,text,text,text,text,jsonb
) from public;
grant execute on function public.post_owner_central_discount_v2(
  text,text,uuid,uuid,timestamptz,numeric,text,text,text,text,jsonb
