-- Emergency duplicate delivered-order correction.
-- Extends the owner-only invoice void so the stock originally deducted by the
-- duplicate order is returned atomically with the financial reversal.
...
begin
  ...
  if v_financial_already_voided and v_order.duplicate_void_inventory_reversed_at is not null then
    raise exception 'This duplicate invoice and its inventory are already reversed.';
  end if;
