-- Repair: support legacy duplicate delivered orders whose inventory deduction is recorded only on order_items.
...
alter table public.owner_inventory_reversals
  ...
  check (reversal_source in ('PICKING_LOCATION', 'LEGACY_COUNTRY_LOCATION', 'LEGACY_ORDER_ITEM_STOCK', 'LEGACY_PRODUCT_STOCK'));
...
create unique index if not exists owner_inventory_reversals_legacy_order_item_uidx
  on public.owner_inventory_reversals(order_id, product_id, location_id, reversal_source)
  where reversal_source = 'LEGACY_ORDER_ITEM_STOCK';
...
declare
  ...
  v_legacy_order_item_reversed_qty numeric := 0;
...
begin
  ...
  -- Some legacy orders pre-date both location-aware picking events and exact
  -- SALE movements. For those orders, order_items.stock_before > stock_after is
  -- persisted proof that stock was deducted. Return exactly that delta to the
  -- one active warehouse matching the order's persisted customer country.
  -- Never guess when the country has zero or multiple active stock rows.
  for s in
    select
      coalesce(oi.replacement_product_id, oi.product_id) as product_id,
      sum(greatest(coalesce(oi.stock_before, 0) - coalesce(oi.stock_after, 0), 0))::numeric as quantity
    from public.order_items oi
    where oi.order_id = v_order.id
      and oi.stock_before is not null
      and oi.stock_after is not null
      and oi.stock_before > oi.stock_after
    group by coalesce(oi.replacement_product_id, oi.product_id)
  loop
    if s.product_id is not null
       and s.quantity > 0
       and not exists (
         select 1 from public.owner_inventory_reversals r
         where r.order_id = v_order.id
           and r.product_id = s.product_id
           and r.reversal_source in ('PICKING_LOCATION', 'LEGACY_COUNTRY_LOCATION', 'LEGACY_ORDER_ITEM_STOCK')
       ) then
      select count(*), min(pls.location_id)
        into v_country_location_count, v_country_location_id
      from public.product_location_stock pls
      ...
      if v_country_location_count <> 1 or v_country_location_id is null then
        raise exception 'Cannot safely return legacy order-item inventory for product %: expected exactly one active % stock location, found %.', s.product_id, v_order_country, v_country_location_count;
      end if;
      ...
      if not found then
        raise exception 'Cannot safely return legacy order-item inventory: location stock row disappeared for product %.', s.product_id;
      end if;
      ...
      insert into public.owner_inventory_reversals (
        ...
        v_correction_id, v_order.id, v_order.order_number, s.product_id,
        v_country_location_id, null, 'LEGACY_ORDER_ITEM_STOCK', s.quantity,
        v_stock_before, v_stock_after, trim(p_reason), v_actor_name, v_actor.staff_id
      );
      ...
      insert into public.stock_movements (
        ...
        v_stock_before,
        v_stock_after,
        'Location inventory return / ' || v_order.order_number || ' / ' || v_order_country
      );

      v_legacy_order_item_reversed_qty := v_legacy_order_item_reversed_qty + s.quantity;
    end if;
  end loop;
  ...
  if v_financial_already_voided
     ...
     and v_legacy_order_item_reversed_qty = 0
  ...
  return jsonb_build_object(
    ...
    'legacy_order_item_returned_qty', v_legacy_order_item_reversed_qty,
