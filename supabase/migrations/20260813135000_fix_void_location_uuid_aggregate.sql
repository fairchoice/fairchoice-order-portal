-- Fix UUID aggregate error in emergency void inventory-location selection.
-- PostgreSQL has no min(uuid), so cast location_id to text for deterministic single-row selection.
-- This replaces only the owner void RPC; no business rows are changed by applying the migration.
...
begin
  ...
  loop
    if not exists (
      ...
      select count(*), min(pls.location_id::text)::uuid
