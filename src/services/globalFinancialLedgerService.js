...
export async function listGlobalFinancialHistory({
  ...
  if (clean.transactionType) {
    query = query.eq("transaction_type", clean.transactionType);
  }
  if (clean.customerAccountId) {
    query = query.eq("customer_account_id", clean.customerAccountId);
  }
  if (clean.customerBranchId) {
    query = query.eq("customer_branch_id", clean.customerBranchId);
  }
  if (clean.dateFrom) {
    query = query.gte("transaction_date", `${clean.dateFrom}T00:00:00`);
  }
  if (clean.dateTo) {
    query = query.lte("transaction_date", `${clean.dateTo}T23:59:59.999`);
  }
  if (clean.search) {
    const escaped = clean.search.replace(/[,%]/g, " ").trim();
    if (escaped) {
      query = query.or(
        `reference.ilike.%${escaped}%,description.ilike.%${escaped}%,staff_name.ilike.%${escaped}%,source_id.ilike.%${escaped}%`
      );
    }
  }
  ...
  return {
    records: (data || []).map(normalizeLedgerRecord),
    total,
    page: currentPage,
    pageSize: size,
...
export async function bulkArchiveFinancialTransactions({
  ...
  const { data, error } = await supabase.rpc("archive_financial_transactions", {
    p_transaction_ids: ids,
    p_actor: currentUser.username,
...
export async function restoreFinancialTransaction({
  ...
  const { data, error } = await supabase.rpc("restore_financial_transaction", {
...
export async function permanentlyDeleteFinancialArchive({
  ...
  if (!String(reason || "").trim()) {
    throw new Error("Permanent delete reason is required.");
  }

  const { data, error } = await supabase.rpc("permanently_delete_financial_archive", {
    p_archive_id: archiveId,
    p_actor: currentUser.username,
    p_reason: String(reason).trim(),
  });
