...
function suggestion(row, leadDays, safetyDays) {
  const daily = Number(row.soldLast30 || 0) / 30;
  ...
  const learningDays = daily > 0 && topups > 0 ? Math.min(2, Math.max(.5, (topups / daily) * .1)) : 0;
  const targetDays = Number(leadDays) + Number(safetyDays) + learningDays;
  const target = Math.ceil(daily * targetDays + Math.max(0, Number(row.preOrderOutstandingQty || 0)));
  ...
  return { ...row, daily, targetDays, target, available, suggested, daysCover, risk, quality, topups };
}
...
export default function PurchasePlanningReport({ products = [], currentUser = null, fetchProducts }) {
  ...
  const visible = useMemo(() => {
    ...
    const sales = (r) => Number(r.soldLast30 || 0);
    const plannedQty = (r) => Number(plan[r.rowKey] ?? r.suggested ?? 0);
    list = [...list].sort((a, b) => {
      if (sortBy === "sales_asc") return sales(a) - sales(b) || a.productName.localeCompare(b.productName);
      if (sortBy === "qty_desc") return plannedQty(b) - plannedQty(a) || sales(b) - sales(a);
      if (sortBy === "stock_asc") return Number(a.currentStock || 0) - Number(b.currentStock || 0);
      return sales(b) - sales(a) || plannedQty(b) - plannedQty(a);
    });
    return list;
  ...
  return <div className="pp-shell"><style>{styles}</style>
    ...
    <section className="pp-model"><div><strong>Planning target</strong><span>Demand × lead time + safety cover, less current/incoming stock. Previous top-ups add only a small learning buffer.</span></div><label>Lead days<input type="number" min="1" max="30" value={leadDays} onChange={(e) => setLeadDays(Math.max(1, Number(e.target.value) || 1))}/></label><label>Safety days<input type="number" min="0" max="30" value={safetyDays} onChange={(e) => setSafetyDays(Math.max(0, Number(e.target.value) || 0))}/></label></section>
    ...
    {loading ? <div className="pp-empty">Loading…</div> : tab === "stockin" ? <section className="pp-stockin">
      ...
      {selectedStockRow && <><div className="pp-stock-card"><strong>{selectedStockRow.productName}</strong><span>{selectedStockRow.productCode} · Current {qty(selectedStockRow.currentStock)} · 30d sold {qty(selectedStockRow.soldLast30)}</span></div>
    ...
    </section> : tab === "history" ? <div className="pp-table"><table><thead><tr><th>Product</th><th>30d Sales</th><th>Main Supplier 30d</th><th>POS / Top-up 30d</th><th>Main Supplier 90d</th><th>POS / Top-up 90d</th><th>Current Stock</th><th>Quality</th></tr></thead><tbody>{pageRows.map((r) => <tr key={r.rowKey}><td><strong>{r.productName}</strong><small>{r.productCode} · {r.country}</small></td><td>{qty(r.soldLast30)}</td><td>{qty(r.supplierPurchased30)}</td><td>{qty(Number(r.topUpReceived30 || 0) + Number(r.preOrderBoughtQty || 0))}</td><td>{qty(r.supplierPurchased90)}</td><td>{qty(r.topUpReceived90)}</td><td>{qty(r.currentStock)}</td><td><Pill>{r.quality}</Pill></td></tr>)}</tbody></table></div> : <div className="pp-table"><table><thead><tr><th>Product</th><th>30d Sales</th><th>Stock</th><th>Incoming</th><th>Days Cover</th><th>POS / Top-up</th><th>Suggested</th><th>This Week Qty</th><th>Risk</th><th>Action</th></tr></thead><tbody>{pageRows.map((r) => <tr key={r.rowKey}><td><strong>{r.productName}</strong><small>{r.productCode} · {r.country} · {r.supplierName}</small></td><td>{qty(r.soldLast30)}</td><td>{qty(r.currentStock)}</td><td>{qty(r.incomingQty)}</td><td>{r.daysCover == null ? "—" : r.daysCover.toFixed(1)}</td><td>{qty(Number(r.topUpReceived30 || 0) + Number(r.preOrderBoughtQty || 0))}</td><td>{qty(r.suggested)}</td><td><div className="pp-step"><button onClick={() => adjust(r.rowKey,-1)}>−</button><input type="number" min="0" value={plan[r.rowKey] ?? 0} onChange={(e) => changeQty(r.rowKey,e.target.value)}/><button onClick={() => adjust(r.rowKey,1)}>+</button></div></td><td><Pill>{r.risk}</Pill></td><td><div className="pp-row-actions"><button onClick={() => openStockIn(r)}>Stock In</button><button className="remove" onClick={() => setRemoved((x) => ({ ...x, [r.rowKey]: true }))}>Remove</button></div></td></tr>)}</tbody></table></div>}
