...
export default function StockTaking({ products = [], fetchProducts }) {
 ...
 const getSystemQty = (product) => {
  const savedKey = `${locationId}:${product.id}`;

  if (Object.prototype.hasOwnProperty.call(savedQuantities, savedKey)) {
    return Number(savedQuantities[savedKey]);
  }
 ...
 try {
  ...
  // Show the newly saved quantities immediately in the System Qty column.
  setSavedQuantities((current) => {
    const updated = { ...current };

    rows.forEach((row) => {
      updated[`${locationId}:${row.productId}`] = Number(row.qty);
    });
  ...
  setMessage(
    `Saved ${rows.length} stock count${rows.length === 1 ? "" : "s"}. Current quantities updated.`
  );
} catch (error) {
