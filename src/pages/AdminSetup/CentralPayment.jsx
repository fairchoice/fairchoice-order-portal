...
function PaymentRecordsPanel({ archived, currentUser, customers, onChanged }) {
  ...
  const filterCustomer = customers.find(
    (customer) => String(customer.id) === String(filters.customerAccountId)
  );
  const filterBranches = getActiveCustomerBranches(filterCustomer);
  ...
  useEffect(() => {
    ...
    void Promise.resolve().then(async () => {
      ...
      try {
        const nextResult = await listCentralPaymentRecords({
          archived,
          ...filters,
          page,
        });
        ...
        if (page > nextResult.total_pages) {
          setPage(nextResult.total_pages);
        }
      } catch (loadError) {
        if (!active) return;
        setMessage(
          loadError.message ||
            "Could not load this payment history page. Please try again."
        );
      } finally {
  ...
  }, [archived, filters, page, refreshKey]);

  const updateFilter = (field, value) => {
    setFilters((current) => ({
      ...current,
      [field]: value,
      ...(field === "customerAccountId" ? { customerBranchId: "" } : {}),
    }));
    setPage(1);
  };
  ...
  return (
    <section className="rounded-2xl border bg-white p-4 shadow-sm">
      ...
      <div className="mb-4 grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-4">
        <select value={filters.customerAccountId} onChange={(event) => updateFilter("customerAccountId", event.target.value)} className="rounded-xl border p-3">
          <option value="">All customers</option>
          {customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.account_name}</option>)}
        </select>
        <select value={filters.customerBranchId} onChange={(event) => updateFilter("customerBranchId", event.target.value)} disabled={!filters.customerAccountId || !filterBranches.length} className="rounded-xl border p-3 disabled:bg-slate-100 disabled:text-slate-400">
          <option value="">All branches</option>
          {filterBranches.map((branch) => <option key={branch.id} value={branch.id}>{branch.branch_name}</option>)}
        </select>
        <select value={filters.transactionType} onChange={(event) => updateFilter("transactionType", event.target.value)} className="rounded-xl border p-3">
      ...
      {loading && <div className="mb-3 rounded-xl bg-blue-50 p-3 font-bold text-blue-800">Loading payment history page...</div>}
      <div className="overflow-x-auto">
        <table className="w-full min-w-[900px] text-sm">
          ...
          <tbody>
            ...
            {!result.records?.length && !loading && !message && <tr><td colSpan="7" className="p-8 text-center text-slate-500">No payment records match these filters.</td></tr>}
          </tbody>
        </table>
      </div>
      <div className="mt-4 flex items-center justify-end gap-3">
        <button type="button" disabled={loading || page <= 1} onClick={() => setPage((value) => value - 1)} className="rounded-lg border px-4 py-2 font-bold disabled:text-slate-300">Previous</button>
        <span className="font-bold">Page {page} of {result.total_pages || 1} · {result.total || 0} transactions</span>
        <button type="button" disabled={loading || page >= (result.total_pages || 1)} onClick={() => setPage((value) => value + 1)} className="rounded-lg border px-4 py-2 font-bold disabled:text-slate-300">Next</button>
      </div>
    </section>
  );
}
...
export default function CentralPayment() {
  ...
  return (
    <div className="space-y-4 p-4">
      ...
      {activeTab === "history" && (
        <PaymentRecordsPanel archived={false} currentUser={currentUser} customers={customers} onChanged={refreshSnapshot} />
      )}
      {activeTab === "archive" && (
        <PaymentRecordsPanel archived currentUser={currentUser} customers={customers} onChanged={refreshSnapshot} />
      )}
