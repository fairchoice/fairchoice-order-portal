...
function hasActivePage(items = [], page) {
  return items.some((item) => item.page === page || hasActivePage(item.children, page));
}
...
function MenuItem({ item, page, depth = 0, onNavigate }) {
  ...
  if (hasChildren) {
    return <div className="bo-nav-branch"><button type="button" className={`bo-tree-toggle bo-nav-parent ${active ? "is-active-parent" : ""}`} onClick={() => setOpen((value) => !value)} aria-expanded={visible} style={{ paddingLeft: `${12 + depth * 14}px` }}><span>{item.label}</span><span className="bo-chevron">{visible ? "-" : "+"}</span></button>{visible && <div className="bo-nav-children">{item.children.map((child) => <MenuItem key={`${item.label}-${child.label}`} item={child} page={page} depth={depth + 1} onNavigate={onNavigate} />)}</div>}</div>;
  }
...
function NavSection({ section, page, onNavigate }) {
  ...
  const visible = open || active;
  return <section className="bo-nav-group"><button type="button" className={`bo-section-toggle ${active ? "is-active-section" : ""}`} onClick={() => setOpen((value) => !value)} aria-expanded={visible}><span>{section.title}</span><span className="bo-chevron">{visible ? "-" : "+"}</span></button>{visible && <div className="bo-section-items">{section.items.map((item) => <MenuItem key={`${section.title}-${item.label}`} item={item} page={page} onNavigate={onNavigate} />)}</div>}</section>;
}
...
.bo-brand { display:flex; gap:12px; align-items:center; padding:18px 16px; border-bottom:1px solid rgba(255,255,255,.12); }
...
.bo-brand-subtitle { font-size:13px; color:rgba(255,255,255,.76); }
...
.bo-section-toggle { min-height:42px; padding:0 10px; border-radius:10px; background:rgba(255,255,255,.07); color:#fff; font-size:12px; font-weight:800; letter-spacing:.08em; text-transform:uppercase; }
.bo-section-toggle:hover,.bo-section-toggle:focus-visible,.bo-tree-toggle:hover,.bo-tree-toggle:focus-visible,.bo-nav-item:hover,.bo-nav-item:focus-visible { background:rgba(255,255,255,.12); outline:none; }
.bo-section-toggle.is-active-section { background:rgba(255,255,255,.16); }
.bo-section-items { padding:6px 0 4px; }
.bo-tree-toggle { min-height:40px; margin:2px 0; border-radius:10px; background:transparent; color:rgba(255,255,255,.82); font-size:14px; font-weight:700; }
.bo-tree-toggle.is-active-parent { color:#fff; background:rgba(255,255,255,.1); }
.bo-nav-item { min-height:40px; margin:2px 0; border-radius:10px; background:transparent; color:rgba(255,255,255,.88); font-size:14px; }
.bo-nav-item.is-active { background:#fff; color:#073763; font-weight:800; box-shadow:0 8px 24px rgba(0,0,0,.16); }
.bo-chevron { flex:0 0 auto; font-size:18px; line-height:1; margin-left:10px; }
.bo-nav-children { margin-left:12px; padding-left:4px; border-left:1px solid rgba(255,255,255,.14); }
.bo-sidebar-footer { padding:14px; border-top:1px solid rgba(255,255,255,.12); }
.bo-role { margin-bottom:10px; color:rgba(255,255,255,.72); font-size:13px; }
.bo-logout { width:100%; min-height:42px; border:1px solid rgba(255,255,255,.28); border-radius:10px; background:rgba(255,255,255,.08); color:#fff; font-weight:700; cursor:pointer; }
...
.bo-header h1 { margin:0; font-size:21px; line-height:1.2; }
.bo-header p { margin:2px 0 0; color:#65758a; font-size:14px; }
...
.bo-placeholder { padding:20px; border-radius:12px; background:#fff; border:1px solid #dfe7f1; }
.bo-placeholder h2 { margin:0 0 8px; font-size:22px; }
.bo-placeholder p { margin:0; color:#65758a; }
