...
function canAccessBackOfficePage(user, page) {
  ...
  if (role === "Brand Partner") {
    return ["order", "brandPerformance"].includes(page);
  }
...
export default function BackOfficeLayout({
  ...
  const allowedSections = useMemo(() => {
    ...
    if (role === "Brand Partner") {
      return sections
        .map((section) => {
          if (section.title === "Order") {
            return {
              ...section,
              items: section.items.filter((item) => item.key === "page.order.sales_rep"),
            };
          }
          if (section.title === "Reports") {
            return {
              ...section,
              items: section.items.filter((item) => item.key === "page.reports.brand_performance"),
            };
          }
