        ...
        <main className="rounded-2xl border bg-white p-4 shadow-sm">
          {!selected.staff_id ? <div className="p-8 text-center text-slate-500">Select a staff member.</div> : <>
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <label className="text-sm font-bold">Role<select disabled={protectedMaster} value={selected.role || "Admin"} onChange={(e) => setField("role", e.target.value)} className="mt-1 w-full rounded-xl border p-2 font-normal">{STAFF_ROLES.map((role) => <option key={role} disabled={role === "Super Admin" && !isMasterAdmin(currentUser)}>{role}</option>)}</select>{selected.role === "Brand Partner" && <span className="mt-1 block text-xs font-normal text-indigo-700">Read-only partner login. Choose the allowed brand in Access Control.</span>}</label>
