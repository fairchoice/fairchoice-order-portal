...
export default function AllCreditOutstanding({ customers = [] }) {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return allTransactions
      ...
      .filter((transaction) => {
        ...
        return `${transaction.customerName} ${transaction.reference} ${transaction.description} ${transaction.type} ${transaction.status}`.toLowerCase().includes(needle);
      })
      .sort((a, b) => new Date(b.date || 0).getTime() - new Date(a.date || 0).getTime());
  }, [allTransactions, historyCustomer, search]);
  ...
  useEffect(() => setHistoryPage(1), [search, historyCustomer]);
    ...
    <div className="rounded-2xl border bg-white p-4 shadow-sm">
      ...
      {activeTab === "outstanding" ? (
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search customer, reference or description" className="rounded-xl border p-3" />
